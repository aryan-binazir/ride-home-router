...
The application must be able to read the credential to call Google, so this is write-only access through the application, not a one-way hash. Database operators and backups remain trusted and must be protected as secrets. The key is not part of public settings, app exports or workflow data. Credential queries and writes report generic errors; no keys are written to application logs. A rollback refuses to drop a populated credential table; explicitly delete the key first if rolling back is intended.
