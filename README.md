...
The server uses Google Maps Platform for everything location related: Places Autocomplete for address suggestions, the Geocoding API for coordinates, and the Routes API for drive times. One Google Maps key, configured by an admin in Settings, covers all three. Routes are planned locally from straight-line estimates, then each car's finished route is measured with Compute Routes (one request per car plus one direct trip for the detour figure), so Google usage grows with the number of cars rather than the square of the roster and stays inside the free monthly allowance. Measured times are shown for the current request only; saved events keep who rides with whom and in what order, and timings can be fetched per car on demand. Coordinates of the people and places used in a plan are refreshed when they are older than 30 days; if that lookup fails the plan still runs on the previous coordinates. A Postgres ledger reserves every Google request first and stops at 8,000 per month per API (months roll over at midnight Pacific time, like Google's free allowances); when deploying mid-month, set `GOOGLE_USAGE_SEED` (for example `routes=2026-09:7000,geocoding=2026-09:120`) so the current month starts from what was already spent. Entries for other months are ignored, so the variable is safe to leave in place. `ROUTING_ENGINE=matrix` selects the legacy distance-matrix planner for quality comparison only. `make eval` runs the planner evaluation suite: many synthetic rosters of different shapes (Chapel Hill and Carrboro heavy, Carrboro and Durham mixes, Durham heavy, Triangle-wide, Raleigh heavy, drivers living far from riders, vans, tight seats, a Chapel Hill venue) at 50 to 500 riders, three seeds each, both modes, compared with a committed baseline; a planner change is acceptable only when no roster gets materially worse. CI runs it for pull requests that touch the planner. The run also writes `_scratch/planner-eval-report.md` (summary, every run, and each run's worst cars in plain language) for a human or an agent to judge whether the plans make sense; the gate only proves nothing got worse. The driver-matching phase has a two-second soft time budget, so a very slow host can stop it early; the tolerances absorb that, and solve time is reported but never compared. Settings are shared per deployment. Postgres also stores active route edits, mobile plan drafts, and spreadsheet imports. Any application instance can continue them after a restart; no sticky sessions or application volume are needed. The browser retains desktop selections and UI preferences.
...
| `CREDENTIAL_ENCRYPTION_KEY` | Required server-only base64-encoded 32-byte random key. Use the same value on every replica and keep it outside Postgres. |
...
The Docker image contains the server and `migrate` binaries, supports `amd64` and `arm64`, runs the server as a non-root user, and checks `/healthz`. `/healthz` is a public alias for `/api/v1/ready`. The readiness endpoint requires the applied database migration version to exactly match the image's latest embedded migration. `/api/v1/health` remains a database connectivity check for liveness.

Set `DATABASE_URL`, `ALLOWED_HOSTS`, `CREDENTIAL_ENCRYPTION_KEY`, and all five Clerk/access variables above. After signing in as an admin, configure the Google Maps API key in Settings. The key's Google Cloud project must have the Routes API, Geocoding API, and Places API (New) enabled, and any API restriction on the key must allow all three; without them address suggestions and saves report that address lookup is not configured. Address suggestions display the Google Maps logo as Google's terms require. The platform normally supplies `PORT`.
...
The checked-in `railway.toml` uses the Dockerfile, runs `migrate` before deployment, gates activation on `/healthz`, and allows 30 seconds for draining. Set `DATABASE_URL` to the shared Postgres service and `ALLOWED_HOSTS` to the application hostname plus `healthcheck.railway.app`. Railway supplies `PORT`. Configure the five Clerk/access variables above on the application service, with the same values on every replica. Keep the image's default start command and do not attach an application volume. See [Railway configuration](https://docs.railway.com/config-as-code/reference) and [healthchecks](https://docs.railway.com/deployments/healthchecks).
...
Import rows and address jobs are separate database records. Workers claim one address for up to one minute and geocode outside transactions. Shutdown releases a claim; after an abrupt failure, another instance retries it when the lease expires. Each unique address gets at most three worker rounds with up to three Google requests per round (nine requests maximum per import attempt). Round counts are reserved in Postgres before lookup and survive restarts; interrupted rounds count toward the limit. Temporary failures wait at least 30 seconds between rounds, or longer during Google's cooldown. Exhausted addresses are marked failed and excluded from import; retrying requires an explicit new import or reapplying the column mapping. Imports continue without an open browser tab while at least one instance is running. All instances share one Google cooldown: when Google answers with a quota or availability error, every instance pauses address lookup until the cooldown passes. Google Routes distance caching remains in Postgres.
...
Before starting this version, generate `CREDENTIAL_ENCRYPTION_KEY` once with `openssl rand -base64 32` and store its output as a server environment secret on every replica. Keep a protected copy in your password manager or secret manager; never commit it or store it in Postgres. Missing or malformed values prevent startup. Do not regenerate it on each deployment. Clerk-managed secrets are unaffected.

This deployment starts without a saved Google key: run the normal schema migrations, start the server with the encryption secret, then enter the Google API key through admin Settings. The schema migration refuses a populated credential table instead of discarding a key; any disposable local credential must be explicitly removed through the previous app before applying it. Stop older replicas during the cutover because their plaintext queries are deliberately incompatible with the new column.

The application decrypts the credential only for server-side use. Encryption protects a database copy when the deployment secret is kept separately; someone controlling the running server can still use its credentials. An incorrect but well-formed encryption secret makes credential reads fail; the admin can still replace or delete the stored key. If the encryption secret is lost, restore it from your protected copy or provision a new one on every replica and re-enter the Google key in Settings. Keep replicas consistent during recovery. This change does not implement automatic encryption-key rotation. The key is not part of public settings, app exports or workflow data. Credential queries and writes report generic errors; no keys are written to application logs. The migration CLI checks for a configured key before beginning a rollback, so refusal leaves the current migration version clean. Explicitly delete the key first if rolling back is intended. The SQL migration also refuses to drop a populated table; stop application writes during rollback, since a concurrent key save or an out-of-band migration runner can still trigger that SQL guard and require migration-state repair.
