...
func TestParticipantRepositorySearch(t *testing.T) {
	store := postgrestest.Open(t)
	repo := store.Participants()
	row, err := repo.Create(t.Context(), &models.Participant{Name: "Zelda", Address: "123 Maple Avenue", AddressName: "Community Center"})
	if err != nil {
		t.Fatal(err)
	}
	other, err := repo.Create(t.Context(), &models.Participant{Name: "Alice", Address: "456 Oak Road"})
	if err != nil {
		t.Fatal(err)
	}
