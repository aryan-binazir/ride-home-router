...
func (r *importJobRepository) Claim(ctx context.Context, token string, ttl time.Duration) (database.ImportJob, bool, error) {
 ...
 WHERE (SELECT next_at FROM provider_throttles WHERE name='google_geocoding')<=clock_timestamp() AND NOT j.done AND (j.claimed_until IS NULL OR j.claimed_until<clock_timestamp()) AND NOT s.consumed AND s.expires_at>clock_timestamp()
...
func (r *importJobRepository) Release(ctx context.Context, job database.ImportJob) error {
	_, err := r.db.ExecContext(ctx, `UPDATE import_jobs SET claim_token=NULL,claimed_until=GREATEST(clock_timestamp()+interval '30 seconds',COALESCE((SELECT next_at FROM provider_throttles WHERE name='google_geocoding'),clock_timestamp())) WHERE session_id=$1 AND job_index=$2 AND claim_token=$3 AND NOT done`, job.SessionID, job.Index, job.Token)
	return err
}
...
func (r *importJobRepository) Finish(ctx context.Context, job database.ImportJob, rows []database.ImportRow, failed bool) error {
	...
	result, err := tx.ExecContext(ctx, `UPDATE import_jobs SET done=true,claim_token=NULL,claimed_until=GREATEST(clock_timestamp()+interval '30 seconds',COALESCE((SELECT next_at FROM provider_throttles WHERE name='google_geocoding'),clock_timestamp())) WHERE session_id=$1 AND job_index=$2 AND claim_token=$3 AND NOT done AND claimed_until>clock_timestamp()`, job.SessionID, job.Index, job.Token)
	if err != nil {
		return err
	}
