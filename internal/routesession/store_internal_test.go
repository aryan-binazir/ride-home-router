...
func TestCreatePastCapacityEvictsLeastRecentlyAccessedSession(t *testing.T) {
	...
	touched := mustCreate(t, store, CreateInput{})
	now = now.Add(time.Second)
	oldest := mustCreate(t, store, CreateInput{})
	now = now.Add(time.Second)
	if _, ok := mustLoad(t, store, touched.ID); !ok {
	...
	now = now.Add(time.Second)
	newest := mustCreate(t, store, CreateInput{})

	if _, ok := mustLoad(t, store, oldest.ID); ok {
...
func TestCreateAtCapacityBreaksAccessTiesBySessionID(t *testing.T) {
	...
	for range MaxConcurrentSessions {
		created := mustCreate(t, store, CreateInput{})
	...
	if _, ok := mustLoad(t, store, oldestID); ok {
...
func TestCommitPastMarkerCapacityEvictsOldestMarker(t *testing.T) {
	...
	for range MaxCommittedSessions + 1 {
		created := mustCreate(t, store, CreateInput{})
		if err := commitSession(store, context.Background(), created.ID, func(context.Context, CommitSnapshot) error { return nil }); err != nil {
	...
	if err := commitSession(store, context.Background(), committedIDs[0], func(context.Context, CommitSnapshot) error { return nil }); !errors.Is(err, ErrNotFound) {
		t.Fatalf("oldest marker Commit() error = %v, want ErrNotFound", err)
	}
	if err := commitSession(store, context.Background(), committedIDs[1], func(context.Context, CommitSnapshot) error { return nil }); !errors.Is(err, ErrAlreadyCommitted) {
...
func TestDeleteExpiredSkipsBusySession(t *testing.T) {
	...
	store := newStore(nil, time.Minute, time.Hour, func() time.Time { return now })
	t.Cleanup(store.Close)
	busy := mustCreate(t, store, CreateInput{})
	other := mustCreate(t, store, CreateInput{})
	...
	if _, ok := mustLoad(t, store, other.ID); !ok {
...
func TestFailedCommitRefreshesSessionTTL(t *testing.T) {
	...
	err := commitSession(store, context.Background(), created.ID, func(context.Context, CommitSnapshot) error {
...
func TestCommittedMarkerExpiresWithSessionTTL(t *testing.T) {
	...
	if err := commitSession(store, context.Background(), created.ID, func(context.Context, CommitSnapshot) error { return nil }); !errors.Is(err, ErrAlreadyCommitted) {
		t.Fatalf("second Commit error = %v, want ErrAlreadyCommitted", err)
	}

	now = now.Add(time.Minute + time.Second)
	store.deleteExpired(now)
	if err := commitSession(store, context.Background(), created.ID, func(context.Context, CommitSnapshot) error { return nil }); !errors.Is(err, ErrNotFound) {
