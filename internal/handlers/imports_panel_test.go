...
func TestImportPanelFlowRendersFragmentsAndRefreshesRoster(t *testing.T) {
	...
	for _, want := range []string{"Looking up addresses…", "<progress", `hx-trigger="every 2s [document.getElementById('import-panel').open]`, `hx-disabled-elt="#import-commit-button"`, `id="import-commit-button"`, `hx-include="#import-selection-form"`, `id="import-selection-form"`, `name="selected"`, "2 of 2 rows selected", "Import 2 rows", "disabled>"} {
