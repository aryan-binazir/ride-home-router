...
func TestLegacyMobileBookmarksUseSharedPages(t *testing.T) {
	mux := setupRoutes(&handlers.Handler{}, web.Static)
	for _, tt := range []struct{ path, target string }{
		{"/m", "/"},
		{"/m/plan/riders", "/"},
		{"/m/people", "/participants"},
		{"/m/people?kind=drivers", "/drivers"},
		{"/m/people/drivers/new", "/drivers"},
		{"/m/places", "/activity-locations"},
		{"/m/places/vans/new", "/vans"},
		{"/m/history/12", "/history"},
		{"/m/desktop-preference?clear=1", "/"},
	} {
		rr := httptest.NewRecorder()
		mux.ServeHTTP(rr, httptest.NewRequestWithContext(t.Context(), http.MethodGet, tt.path, nil))
		if rr.Code != http.StatusSeeOther || rr.Header().Get("Location") != tt.target {
			t.Fatalf("%s: %d %s, want 303 %s", tt.path, rr.Code, rr.Header().Get("Location"), tt.target)
		}
	}
	rr := httptest.NewRecorder()
	mux.ServeHTTP(rr, httptest.NewRequestWithContext(t.Context(), http.MethodPost, "/m/plan/when", strings.NewReader("route_time=06:45")))
	if rr.Code != http.StatusMethodNotAllowed {
		t.Fatalf("retired mutation: %d", rr.Code)
	}
}
