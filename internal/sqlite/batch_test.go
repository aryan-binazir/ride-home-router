...
func TestParticipantCreateBatchRollsBackOnMidBatchFailure(t *testing.T) {
	store := newBatchTestStore(t)
	if _, err := store.db.ExecContext(context.Background(), `
		CREATE TRIGGER fail_participant_batch
	...
	if _, err := store.Participants().CreateBatch(context.Background(), batch, nil); err == nil {
		t.Fatal("CreateBatch() error = nil, want injected failure")
	}
	...
	if len(participants) != 0 {
		t.Fatalf("participants after failed batch = %d, want 0", len(participants))
	}
	for i, participant := range batch {
		if participant.ID != 0 {
			t.Errorf("batch[%d].ID = %d after rollback, want 0", i, participant.ID)
		}
	}
}

func TestDriverCreateBatchRollsBackOnMidBatchFailure(t *testing.T) {
	store := newBatchTestStore(t)
	if _, err := store.db.ExecContext(context.Background(), `
		CREATE TRIGGER fail_driver_batch
	...
	batch := []*models.Driver{
		...
		{Name: "Third", Address: "3 Main St", Lat: 40.3, Lng: -73.3, VehicleCapacity: 4},
	}
	if _, err := store.Drivers().CreateBatch(context.Background(), batch, nil); err == nil {
		t.Fatal("CreateBatch() error = nil, want injected failure")
	}
	...
	if len(drivers) != 0 {
		t.Fatalf("drivers after failed batch = %d, want 0", len(drivers))
	}
}
...
func TestCreateBatchRechecksNormalizedDuplicatesInsideTransaction(t *testing.T) {
	...
	participantsAfter, err := store.Participants().List(ctx, "")
	if err != nil || len(participantsAfter) != 3 {
		t.Fatalf("participants after batch = %d, err=%v, want 3", len(participantsAfter), err)
	}
	driversAfter, err := store.Drivers().List(ctx, "")
	if err != nil || len(driversAfter) != 2 {
		t.Fatalf("drivers after batch = %d, err=%v, want 2", len(driversAfter), err)
	}
}
...
func newBatchTestStore(t *testing.T) *Store {
	t.Helper()
	store, err := New(filepath.Join(t.TempDir(), "batch-test.db"))
	if err != nil {
