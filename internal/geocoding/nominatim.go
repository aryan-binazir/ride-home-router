...
func (g *nominatimGeocoder) Geocode(ctx context.Context, address string) (*GeocodingResult, error) {
	...
	if err != nil {
		...
		return nil, &ErrGeocodingFailed{address: address, Reason: "provider request failed", Cause: newNominatimTransportError(err)}
	}
...
func (g *nominatimGeocoder) searchOnce(ctx context.Context, query string, limit int) ([]GeocodingResult, error) {
	...
	if err != nil {
		...
		return nil, &ErrGeocodingFailed{address: query, Reason: "provider request failed", Cause: newNominatimTransportError(err)}
	}
	defer func() { _ = resp.Body.Close() }()
...
func decodeNominatimResults(body io.Reader) ([]nominatimResponse, error) {
	...
	if responseBody.Err != nil {
		return nil, newNominatimTransportError(responseBody.Err)
	}
	if err != nil {
		return nil, err
	}
